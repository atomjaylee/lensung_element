...
interface BaseTagProps {
  ...
  style?: string;
  className?: string;
  $slots?: Record<string, any>; // 组件默认插槽，用于动态格式化样式
}

const defaultProps: BaseTagProps = {
  type: 'fill',
  hidden: false,
  fontColor: '#333',
  fillColor: '#f2f2f2',
      ...
      if ($default && Array.isArray($default) && $default.length === 1 && isString($default[0])) {
        ...
        switch (slotText) {
          case '闲鱼':
            ...
            _fillColor = '#ffd944';
            this.setData({ fontColor: _fontColor, fillColor: _fillColor });
            break;
          case '新品':
